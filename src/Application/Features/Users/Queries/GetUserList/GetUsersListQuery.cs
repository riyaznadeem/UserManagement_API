...
namespace Application.Features.Users.Queries.GetUserList
{
    public class GetUsersListQuery : Pagination, IRequest<PaginatedList<GetUserListViewModel>>
    {
        ...
        public string? SortBy { get; set; }           // e.g. "Username", "DisplayName"
    ...
    public class GetUsersListQueryHandler : IRequestHandler<GetUsersListQuery, PaginatedList<GetUserListViewModel>>
    {
        ...
        {
            var query = _context.Users.Include(u => u.Role).Where(w => w.IsActive).AsNoTracking().AsQueryable();
            ...
            query = request.SortBy?.ToLower() switch
            {
                ...
                _ => query.OrderBy(u => u.CreatedDate) // Default sort
            };
            ...
            var pagedResult = await query
                .Select(u => new GetUserListViewModel
                {
                    ...
                    RoleId = u.Role != null ?  u.RoleId : Guid.Empty
                })
